...
for STD_TOPO in STD_TOPOs:
    for STD_TOPO_flag in STD_TOPO_flags:
        ...
        T_ref_values = range(
            int(np.nanmin(T2_54km_3D["T2"]) + 1),
            int(np.nanmax(T2_54km_3D["T2"]) - 1),
            T_bin_size,
        )
        if ini_switch == True:
            ...
            for T_ref in T_ref_values:
                try:
                    ...
                    if plotting_scatter_all:
                        ...
                        plt.title(
                            "WRF 54km - 3km T2 and %s correlation at %s °C T_ref"
        ...
        if T_bin_flag:
            ...
            figname = (
                outfolder
                + f"WRF_T_ref_coefficients_STD_{STD_TOPO}_{hour_start}-{hour_end}h_till_{end_date}.png"
            )
            plt.savefig(figname)
            plt.close()
